...
type KnobBaseThumbProps = {
  readonly theme: 'stone' | 'pink' | 'green' | 'sky' | 'red';
...
export function KnobBaseThumb({theme, value01}: KnobBaseThumbProps) {
  ...
  return (
    <div
      className={clsx(
        'absolute h-full w-full rounded-full hover:bg-red-800 hover:cursor-ns-resize',
    ...
    >
      <div className='absolute h-full w-full' style={{rotate: `${angle}deg`}}>
        <div className='absolute left-1/2 top-0 h-1/2 w-[2px] -translate-x-1/2 rounded-sm bg-stone-300' />
      </div>
    </div>
  );
}